...
class HomeRepository{
	...
	public function getTotalRain($search){
		$totalRain = Weather::whereYear('date', date('Y'))->where('datetime', '<=', date('Y-m-d H:i:s', strtotime($search)))->where('hi_temp', '>', 0)->sum('rain');
	...
	public function getCurrentRain($search){
        $currentRain = Weather::where('datetime', '>', date('Y-m-d H:i:s', strtotime('-15 min')))->where('datetime', '<=', date('Y-m-d H:i:s'))->where('hi_temp', '>', 0)->avg('rain');

        return $currentRain;
	}

	public function getPrograssiveRain($search){
		return Weather::where('datetime', '>=', date('Y-m-d H:i:s', strtotime($search)))->where('datetime', '<=', date('Y-m-d H:i:s'))->where('hi_temp', '>', 0)->avg('rain');
	}
