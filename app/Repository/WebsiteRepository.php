...
class WebsiteRepository{
	public function getPollutionAQI(){
		...
		foreach($locations as $location){
			$aqis = AirQualityIndex::where('pollution_location_id', $location->id)
				  ->select('date', 'so2', 'nox', 'pm2', 'rspm', 'co', 'o3', 'nh3')
				  ->latest()
				  ->take(5)
				  ->get();
			...
			foreach($aqis as $aqi){
				$aqiArr = [];
				$aqiArr = [$this->getSo2AirQualityIndexAttr($aqi->so2),$this->getNoxAirQualityIndexAttr($aqi->nox),$this->getPm2AirQualityIndexAttr($aqi->pm2),$this->getRspmAirQualityIndexAttr($aqi->rspm),$this->getCoAirQualityIndexAttr($aqi->co),$this->getO3AirQualityIndexAttr($aqi->o3),$this->getNh3AirQualityIndexAttr($aqi->nh3)];
	...
	public function pollutionUpdateChart($req){
		...
		foreach($pollutantLocations as $pollutantLocation){
			...
			}elseif($req->selectPollutant == "rspm"){
				$aqi = $this->getRspmAirQualityIndexAttr($pollutantLocation->data);
			}elseif($req->selectPollutant == "co"){
				$aqi = $this->getCoAirQualityIndexAttr($pollutantLocation->data);
			}elseif($req->selectPollutant == "o3"){
				$aqi = $this->getO3AirQualityIndexAttr($pollutantLocation->data);
			}elseif($req->selectPollutant == "nh3"){
				$aqi = $this->getNh3AirQualityIndexAttr($pollutantLocation->data);
			}
	...
	public function pollutionUpdatePieChart($req){
		$pollutantLocations = AirQualityIndex::join('pollution_locations', 'pollution_locations.id', '=', 'air_quality_index.pollution_location_id')
						    ...
						    ->select('air_quality_index.date', 'air_quality_index.so2', 'air_quality_index.nox', 'air_quality_index.pm2', 'air_quality_index.rspm', 'air_quality_index.co', 'air_quality_index.o3', 'air_quality_index.nh3')
						    ->get();
		// return 
		$aqiDataArr = [];
		foreach($pollutantLocations as $pollutantLocation){
			$aqiArr = [];
			$aqiArr = [$this->getSo2AirQualityIndexAttr($pollutantLocation->so2),$this->getNoxAirQualityIndexAttr($pollutantLocation->nox),$this->getPm2AirQualityIndexAttr($pollutantLocation->pm2),$this->getRspmAirQualityIndexAttr($pollutantLocation->rspm),$this->getCoAirQualityIndexAttr($pollutantLocation->co),$this->getO3AirQualityIndexAttr($pollutantLocation->o3),$this->getNh3AirQualityIndexAttr($pollutantLocation->nh3)];
	...
	// to get the pm10/rspm aqi
	public function getRspmAirQualityIndexAttr($val){
		$aqi = 0;
		if($val <= 100){
			$aqi = $val;
